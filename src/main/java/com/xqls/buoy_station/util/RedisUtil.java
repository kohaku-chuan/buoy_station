...
public class RedisUtil {
    ...
    public void init() {
        redisUtil = this;
        redisUtil.template = this.template;
    }
    ...
    public boolean llSet(String key, Object value) {
        try {
            template.opsForList().leftPush(key, (String) value);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public boolean channelIsUnblocked(String key) {
        if (!template.hasKey(key)) {
            return true;
        } else {
            String value = template.opsForValue().get(key).toString();
